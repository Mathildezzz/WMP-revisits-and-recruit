...
select  
     ...
     NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' then trans.member_detail_id else null end)),0)                                                                                                                AS wos_member_shopper_from_wmp,
     CAST((sum(case when is_member_order IS TRUE AND sales_qty > 0 AND order_scenario = 'Organic' then order_rrp_amt else 0 end) - sum(case when is_member_order IS TRUE AND sales_qty < 0 AND order_scenario = 'Organic' then abs(order_rrp_amt) else 0 end)) AS FLOAT)                AS wos_member_sales_from_wmp,
     CAST((sum(case when sales_qty > 0 AND order_scenario = 'Organic' then order_rrp_amt else 0 end) - sum(case when sales_qty < 0 AND order_scenario = 'Organic' then abs(order_rrp_amt) else 0 end)) AS FLOAT)                                                                        AS wos_ttl_sales_from_wmp,
     CAST((sum(case when is_member_order IS TRUE AND sales_qty > 0 AND order_scenario = 'Organic' then order_rrp_amt else 0 end) - sum(case when is_member_order IS TRUE AND sales_qty < 0 AND order_scenario = 'Organic' then abs(order_rrp_amt) else 0 end)) AS FLOAT)/NULLIF(count(distinct case when is_member_order IS TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' then trans.parent_order_id else null end),0)   AS wos_member_atv_from_wmp,
     CAST((count(distinct case when is_member_order IS TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' then trans.parent_order_id else null end)) AS FLOAT) / NULLIF((count(distinct case when is_member_order IS TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' then trans.member_detail_id else null end)) ,0)                                                                     AS wos_member_frequency_from_wmp,
     
     NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' AND eff_reg_channel = 'BRANDWMP' then trans.member_detail_id else null end)),0)                           AS wos_member_shopper_wmp_reg,
     NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' AND eff_reg_channel LIKE '%LCS%' then trans.member_detail_id else null end)),0)                           AS wos_member_shopper_lcs_reg,

     NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' AND eff_reg_channel = 'BRANDWMP' AND new_member.member_detail_id IS NOT NULL then trans.member_detail_id else null end)),0)                           AS wos_member_shopper_wmp_reg_new,
     NULLIF((count(distinct case when is_member_order = TRUE AND if_eff_order_tag = true AND order_scenario = 'Organic' AND eff_reg_channel LIKE '%LCS%' AND new_member.member_detail_id IS NOT NULL then trans.member_detail_id else null end)),0)                           AS wos_member_shopper_lcs_reg_new

from omni_trans_fact trans
LEFT JOIN new_member
      ON trans.member_detail_id::integer = new_member.member_detail_id::integer
where 1 = 1
